...
namespace _11_ArraysAndListsChallenge
{
    public class Program
    {
        ...
        public static int[] SunIsShining(int[] x)
        {
            ...
            {
                x[i] += 2; //increments each found array int by 2
            }
            return x;
        }
        ...
        public static decimal ArrayListAvg(ArrayList myArrayList)
        {
            ...
            decimal count = 0;
            List<decimal> DoubInt = new List<decimal>();
            foreach (object typ in myArrayList) //Must be object, not type 
            {
                if(typ.GetType() == typeof(string))
                {
                    //strings are not counted, so this if statement will do nothing
                }
                else if (Decimal.TryParse(typ.ToString(), out decimal dec))
                {
                    count += dec;
                    DoubInt.Add(dec);
                }
            }
            decimal avg = count / DoubInt.Count;
            avg = decimal.Round(avg, 3, MidpointRounding.ToEven);
            return avg;
        }
        ...
        public static int ListAscendingOrder(List<int> scores, int yourScore)
        {
            ...
            return ++rank; //increment rank based on int's indexed position
        }
